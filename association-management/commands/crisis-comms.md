# /crisis-comms

Develop crisis communication plans and manage urgent communications.

## Usage

```
/crisis-comms [mode] [options]
```

## Arguments

- `mode`: Operating mode (plan-development, active-response, post-crisis, simulation)
- `--crisis-type`: Type of crisis (reputational, operational, industry, leadership, financial, safety, cyber)
- `--severity`: Crisis severity level (1-low to 5-critical)
- `--stakeholders`: Primary affected stakeholders
- `--timeline`: Response timeline or plan horizon

## What This Command Does

Prepare for and manage crisis communications:

### 1. Crisis Communication Plan Development

**Plan Structure**

*Purpose and Scope*
- Plan objectives
- Crisis definition
- Activation criteria
- Plan ownership

*Crisis Classification*
- Severity levels (1-5)
- Crisis categories
- Escalation triggers
- Response protocols by level

*Roles and Responsibilities*
- Crisis management team
- Spokesperson designation
- Decision authority matrix
- Backup personnel
- External resources

*Communication Protocols*
- Internal notification chain
- External communication approval
- Media response procedures
- Social media protocols
- Stakeholder priority matrix

### 2. Crisis Categories & Scenarios

**Reputational Crisis**
- Member misconduct
- Staff/leadership issues
- Controversial positions
- Social media incidents
- Negative media coverage

**Operational Crisis**
- Data breach/cyber incident
- Service disruption
- Venue/event emergency
- Vendor failure
- Technology failure

**Industry Crisis**
- Regulatory action
- Public safety concerns
- Economic disruption
- Competitive threat
- Public perception shift

**Financial Crisis**
- Revenue shortfall
- Fraud/misappropriation
- Investment loss
- Bankruptcy/closure
- Audit issues

**Leadership Crisis**
- Executive departure
- Board conflict
- Succession emergency
- Misconduct allegations
- Health emergency

**Safety/Security**
- Physical threat
- Natural disaster
- Workplace violence
- Event emergency
- Public health issue

### 3. Stakeholder Communication

**Stakeholder Priority Matrix**

*Tier 1 - Immediate Notification*
- Board of Directors
- Executive team
- Legal counsel
- Key staff

*Tier 2 - Rapid Communication*
- Members/customers
- Major sponsors/partners
- Regulatory bodies
- Employee full team

*Tier 3 - Timely Update*
- Media
- General public
- Vendors
- Allied organizations

**Communication Channels by Stakeholder**
- Direct phone/text for urgent
- Email for documentation
- Website for public record
- Social media for broad reach
- Media statements for press
- Member portal for members

### 4. Message Development

**Core Message Framework**
- What happened (facts only)
- What we're doing about it
- What stakeholders should do
- Where to get more information
- Expression of appropriate concern

**Message Components**

*Initial Statement*
- Acknowledge situation
- Express appropriate concern
- State commitment to address
- Promise updates
- Provide contact information

*Detailed Statement*
- Full situation explanation
- Timeline of events
- Actions being taken
- Impact mitigation
- Ongoing commitment

*Updates*
- Progress on resolution
- New information
- Revised guidance
- Continued commitment

**Tone Guidelines**
- Empathetic but professional
- Factual, avoid speculation
- Take appropriate responsibility
- Forward-looking focus
- Consistent across channels

### 5. Media Relations

**Media Response Protocol**
- Spokesperson designation only
- Statement approval process
- No off-the-record comments
- Interview preparation
- Reporter relationship management

**Press Materials**
- Press release template
- Q&A document
- Background/fact sheet
- Spokesperson talking points
- Media advisory format

**Interview Preparation**
- Key messages (3 max)
- Bridging techniques
- Anticipated questions
- No-go areas
- Visual presentation

### 6. Social Media Protocol

**Monitoring**
- Keyword tracking
- Mention alerts
- Sentiment analysis
- Influencer identification

**Response Strategy**
- Response decision tree
- Tone guidelines
- Escalation criteria
- Community management

**Content Management**
- Pause scheduled content
- Pin crisis information
- Consistent messaging
- Monitor and respond

### 7. Active Crisis Response

**First Hour Checklist**
- Activate crisis team
- Gather initial facts
- Assess severity level
- Notify key stakeholders
- Prepare holding statement
- Monitor media/social

**First Day Actions**
- Detailed situation assessment
- Stakeholder communication execution
- Media statement release
- Internal team briefing
- Legal consultation
- Documentation initiation

**Ongoing Management**
- Regular status meetings
- Stakeholder updates
- Media monitoring
- Message refinement
- Resolution tracking
- Documentation

### 8. Recovery & Learning

**Post-Crisis Actions**
- Resolution communication
- Thank you to supporters
- Lessons learned documentation
- Process improvements
- Relationship repair
- Plan updates

**After-Action Review**
- Timeline reconstruction
- Response evaluation
- Communication effectiveness
- Gap identification
- Recommendation development

**Plan Updates**
- Scenario additions
- Contact list updates
- Protocol refinements
- Training needs
- Resource requirements

### 9. Crisis Simulation

**Tabletop Exercise Design**
- Scenario development
- Participant selection
- Facilitation guide
- Evaluation criteria
- Debrief protocol

**Drill Components**
- Notification testing
- Decision-making simulation
- Message development practice
- Media simulation
- Stakeholder role-play

## Best Practices Applied

- Prepare before crisis hits
- Communicate early and often
- Be honest and transparent
- Show empathy and concern
- Maintain consistent messages
- Document everything
- Learn and improve
- Practice regularly

## Templates Included

**Crisis Plan Document**
- Complete plan structure
- Role assignments
- Contact trees
- Protocol checklists

**Response Templates**
- Initial holding statement
- Media statement formats
- Social media responses
- Internal communications
- Stakeholder letters

**Operational Tools**
- Crisis assessment form
- Decision log template
- Communication tracker
- After-action review form

## Output Formats

- Complete crisis communication plan
- Scenario-specific response guides
- Message templates
- Media materials
- Simulation exercise materials

## Example

```
/crisis-comms plan-development --crisis-type="cyber,reputational,leadership" --stakeholders="members,sponsors,media,regulators" --timeline="12-month plan with quarterly reviews"
```

Develops a comprehensive crisis communication plan covering cyber incidents, reputational threats, and leadership crises with multi-stakeholder protocols.
