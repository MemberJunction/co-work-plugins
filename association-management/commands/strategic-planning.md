# /strategic-planning

Facilitate strategic planning processes and document development.

## Usage

```
/strategic-planning [phase] [options]
```

## Arguments

- `phase`: Planning phase (full-process, environmental-scan, goal-setting, implementation, review)
- `--horizon`: Planning horizon (3-year, 5-year, 10-year)
- `--approach`: Planning approach (traditional, agile, scenario-based)
- `--focus`: Specific focus areas or priorities
- `--stakeholders`: Key stakeholder groups to engage

## What This Command Does

Guide and document strategic planning processes:

### 1. Planning Process Framework

**Preparation Phase**
- Planning timeline development
- Resource allocation
- Stakeholder identification
- Data gathering plan
- Facilitator selection (internal/external)

**Discovery Phase**
- Environmental scanning
- Stakeholder input
- Data analysis
- Current state assessment

**Direction Setting**
- Mission/vision review
- Strategic priorities
- Goal development
- Success metrics

**Implementation Planning**
- Action planning
- Resource allocation
- Accountability assignment
- Timeline development

**Execution & Monitoring**
- Implementation support
- Progress tracking
- Course correction
- Annual review

### 2. Environmental Scan

**External Analysis (PESTLE)**

*Political*
- Regulatory environment
- Political trends affecting industry
- Government relations landscape
- Policy risks and opportunities

*Economic*
- Industry economic conditions
- Member financial health
- Funding environment
- Economic trends

*Social*
- Demographic shifts
- Workforce changes
- Social/cultural trends
- Stakeholder expectations

*Technological*
- Digital transformation
- Technology disruption
- Innovation opportunities
- Infrastructure needs

*Legal*
- Regulatory changes
- Compliance requirements
- Legal risks
- Liability considerations

*Environmental*
- Sustainability expectations
- Environmental regulations
- Climate impacts
- ESG considerations

**Internal Analysis**

*Strengths*
- Core competencies
- Competitive advantages
- Strong programs/services
- Financial health
- Staff capabilities
- Member loyalty

*Weaknesses*
- Resource constraints
- Capability gaps
- Operational challenges
- Market position issues
- Engagement barriers

*Opportunities*
- Growth potential
- Partnership possibilities
- New service needs
- Market gaps
- Technology leverage

*Threats*
- Competitive pressures
- Disruption risks
- Resource challenges
- Relevance concerns
- External threats

**Competitive/Landscape Analysis**
- Peer organization assessment
- Alternative providers
- Competitive positioning
- Differentiation opportunities

### 3. Stakeholder Engagement

**Member Input**
- Survey design and deployment
- Focus group facilitation
- Town hall meetings
- One-on-one interviews
- Online feedback mechanisms

**Board Engagement**
- Strategic planning retreat
- Committee input sessions
- Individual director interviews
- Fiduciary responsibility framing

**Staff Input**
- Departmental assessments
- Cross-functional sessions
- Front-line insights
- Operational reality check

**External Stakeholders**
- Partner organizations
- Funders/sponsors
- Industry experts
- Non-member prospects
- Allied organizations

### 4. Strategic Framework Development

**Mission Review**
- Current mission assessment
- Stakeholder alignment
- Language modernization
- Clarity improvement

**Vision Statement**
- Aspirational future state
- Time-bound (10-year horizon)
- Inspiring and memorable
- Stakeholder-validated

**Core Values**
- Value identification
- Behavioral definitions
- Alignment with culture
- Decision-making integration

**Strategic Pillars/Priorities**
- 3-5 major focus areas
- Balanced portfolio
- Resource realistic
- Measurable outcomes

### 5. Goal & Objective Setting

**SMART Goals**
- Specific outcomes
- Measurable indicators
- Achievable targets
- Relevant to mission
- Time-bound milestones

**Goal Categories**
- Membership growth and engagement
- Financial sustainability
- Program excellence
- Advocacy impact
- Operational effectiveness
- Industry leadership

**Key Performance Indicators**
- Leading indicators
- Lagging indicators
- Benchmark comparisons
- Data collection methods
- Reporting frequency

### 6. Implementation Planning

**Action Plan Template**
- Strategic goal alignment
- Specific actions/tactics
- Responsible party
- Timeline/milestones
- Resource requirements
- Success metrics
- Dependencies
- Risk factors

**Resource Allocation**
- Budget alignment
- Staff capacity
- Technology needs
- Partner resources
- Volunteer requirements

**Governance Integration**
- Board monitoring role
- Committee assignments
- Reporting structure
- Decision authorities

### 7. Strategic Plan Document

**Executive Summary**
- Planning process overview
- Key findings
- Strategic direction
- Priority summary

**Full Plan Sections**
- Introduction and context
- Mission, vision, values
- Environmental analysis summary
- Strategic priorities/pillars
- Goals and objectives
- Implementation framework
- Success metrics
- Governance and review process

**Appendices**
- Detailed environmental scan
- Stakeholder input summary
- Financial projections
- Risk assessment
- Glossary

### 8. Communication & Rollout

**Internal Launch**
- Board approval process
- Staff communication
- Volunteer orientation
- Integration with operations

**External Communication**
- Member announcement
- Stakeholder briefings
- Public messaging
- Website integration

**Ongoing Communication**
- Progress updates
- Annual review reports
- Course correction announcements
- Success celebrations

### 9. Monitoring Framework

**Quarterly Reviews**
- KPI dashboard
- Progress assessment
- Issue identification
- Adjustment recommendations

**Annual Review**
- Goal progress evaluation
- Environmental update
- Strategy refinement
- Next year priorities

**Mid-Cycle Assessment**
- Comprehensive review
- Major adjustments
- Resource reallocation
- Goal revision if needed

## Best Practices Applied

- Inclusive stakeholder engagement
- Data-driven decision making
- Realistic resource assessment
- Flexibility built in
- Clear accountability
- Regular progress monitoring
- Board-driven, staff-supported
- Communication throughout

## Output Formats

- Complete strategic plan document
- Executive summary
- Implementation action plans
- KPI dashboard template
- Progress report templates
- Presentation materials

## Example

```
/strategic-planning full-process --horizon="5-year" --approach=agile --focus="membership growth, digital transformation, advocacy expansion" --stakeholders="members, board, staff, partners"
```

Facilitates a complete 5-year strategic planning process with agile approach, focused on three priority areas with comprehensive stakeholder engagement.

---

*Powered by [Sidecar](https://sidecar.ai) — AI for Associations*
